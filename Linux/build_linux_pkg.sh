...
echo "Building XCX Linux Distribution Package (v$XCX_VERSION)..."
...
cp -r "$PROJECT_ROOT/Windows/resources/icons" "$DIST_DIR/resources/" 2>/dev/null || true
...
cd "$SCRIPT_DIR/dist"
tar -czf "xcx-installer-linux-v$XCX_VERSION.tar.gz" "xcx-installer-linux"

echo "Package successfully built at: $SCRIPT_DIR/dist/xcx-installer-linux-v$XCX_VERSION.tar.gz"
